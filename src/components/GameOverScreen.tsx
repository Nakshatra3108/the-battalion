...
export default function GameOverScreen({ state, onPlayAgain }: GameOverScreenProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: 'rgba(0, 0, 0, 0.95)' }}>
      ...
      <div className="relative glass-card rounded-lg p-8 max-w-md mx-4 text-center border-2 border-[#4caf50]/50 shadow-[0_0_40px_rgba(76,175,80,0.3)]">
        {/* Corner brackets */}
        ...
        {/* Victory Icon */}
        <div className="text-6xl mb-4 animate-pulse" style={{ filter: 'drop-shadow(0 0 20px rgba(255, 215, 0, 0.5))' }}>⭐</div>

        {/* Winner */}
        <h1 className="text-3xl font-bold mb-2 uppercase tracking-widest glow-text" style={{
        ...
        {winner && (
          <div
            ...
            style={{
              ...
              boxShadow: `0 0 15px ${winner.color}40`
            }}
          >
            <span
        ...
        <button
          onClick={onPlayAgain}
          className="w-full py-3 border-2 border-[#4caf50] text-[#4caf50] font-bold rounded-lg transition-all duration-300 uppercase tracking-widest hover:bg-[#4caf50] hover:text-black hover:shadow-[0_0_20px_rgba(76,175,80,0.5)]"
